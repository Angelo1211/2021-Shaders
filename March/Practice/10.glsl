...
CalcAO(vec3 P, vec3 N)
{
    ...
    return (1.0 - 3.0*occlusion) * (N.y * 0.5 + 0.5);
}
...
Render(vec3 ro, vec3 rd)
{
    ...
    float sky_sunset = ( 0.5 + 0.5*sin(iTime/ 2.0  + PI)) * (rd.y + 0.3) ;
    vec3 sky_color = vec3(0.2, 0.4 *sky_sunset, 0.8 * sky_sunset );
    if(id < 0.0)
    {
        col = sky_color;
    }
    else
    {
        ...
        float diffuse = saturate(dot(L_sun, N));
        ...
        // Direct light
        {
            ...
            acc += 1.0 * specular * vec3(1.0, 0.8, 0.7);
            acc += 1.0 * diffuse * vec3(1.0, 0.8, 0.7);
        }
        ...
        // Sky
        {
            ...
            acc += 0.3 * diffuse * sky_color;
        }
        col *= acc;
    }

    col = mix(col , sky_color, 1.0 - exp(-0.05*t*t*t));

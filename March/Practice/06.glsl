...
CalcMaterial(float id, vec3 P)
{
    ...
    if(id == ID_GROUND)
    {
        vec2 ids = floor(P.xz * 4.0);
        float isWhiteTile = mod(ids.x + ids.y,2.0);
        col = vec3(isWhiteTile) + 0.15;
    }
...
Render(vec3 ro, vec3 rd)
{
    ...
    vec3 skyColor = vec3(0.1, 0.3, 0.9);
    if(id < 0.0) // Sky
    {
        col = skyColor;
    }
    else
    {
        ...
        float specular = pow(saturate(dot(N, H)), 128.0);
        float ambient  = 0.01;
        ...
        // Shading
        acc += 1.0 * diffuse * vec3(0.9, 0.8, 0.9);
        acc += 1.0 * specular * vec3(1.0);
